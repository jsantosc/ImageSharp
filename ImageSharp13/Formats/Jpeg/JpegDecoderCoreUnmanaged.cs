...
namespace ImageSharp.Formats
{
    ...
    /// </summary>
    internal sealed unsafe class JpegDecoderCoreUnmanaged : IDisposable
    {
        /// <summary>
...
#pragma warning disable SA1401 // FieldsMustBePrivate
        ...
        /// </summary>
        public SpanInputProcessor InputProcessor;
#pragma warning restore SA401
        ...
        public JpegDecoderCoreUnmanaged(Configuration configuration, IJpegDecoderOptions options)
        {
        ...
        /// </summary>
        public SpanUnmanagedArray<byte> InputStream { get; private set; }
        ...
        public Image<TPixel> Decode<TPixel>(SpanUnmanagedArray<byte> stream)
        ...
        private void ProcessStream(ImageMetaData metadata, SpanUnmanagedArray<byte> stream, bool metadataOnly)
        {
            this.InputStream = stream;
            this.InputProcessor = new SpanInputProcessor(stream);

            // Check for the Start Of Image marker.
            var bytes = this.InputProcessor.ReadFull(0, 2);
            if (bytes[0] != JpegConstants.Markers.XFF || bytes[1] != JpegConstants.Markers.SOI)
            {
            ...
            while (processBytes)
            {
                bytes = this.InputProcessor.ReadFull(0, 2);
                while (bytes[0] != 0xff)
                {
                    ...
                    bytes = this.InputProcessor.ReadFull(-1, 2);
                }
                ...
                bytes = this.InputProcessor.ReadFull(0, 2);
                int remaining = (bytes[0] << 8) + bytes[1] - 2;
                if (remaining < 0)
                {
        ...
        private void ProcessApp14Marker(int remaining)
        {
            ...
            var bytes = this.InputProcessor.ReadFull(0, 12);
            remaining -= 12;

            if (bytes[0] == 'A' && bytes[1] == 'd' && bytes[2] == 'o' && bytes[3] == 'b'
                && bytes[4] == 'e')
            {
                this.adobeTransformValid = true;
                this.adobeTransform = bytes[11];
            }
        ...
        private void ProcessApp1Marker(int remaining, ImageMetaData metadata)
        {
            ...
            var profile = this.InputProcessor.ReadFull(0, remaining);
        ...
        private void ProcessApp2Marker(int remaining, ImageMetaData metadata)
        {
            ...
            var identifier = this.InputProcessor.ReadFull(0, Icclength);
        ...
        private void ProcessApplicationHeader(int remaining)
        {
            ...
            var bytes = this.InputProcessor.ReadFull(0, 13);
            remaining -= 13;

            // TODO: We should be using constants for this.
            this.isJfif = bytes[0] == 'J' &&
                          bytes[1] == 'F' &&
                          bytes[2] == 'I' &&
                          bytes[3] == 'F' &&
                          bytes[4] == '\x00';
            ...
            if (this.isJfif)
            {
                this.horizontalResolution = (short)(bytes[9] + (bytes[8] << 8));
                this.verticalResolution = (short)(bytes[11] + (bytes[10] << 8));
            }
        ...
        private void ProcessDefineHuffmanTablesMarker(int remaining)
        {
            while (remaining > 0)
            {
                ...
                var bytes = this.InputProcessor.ReadFull(0, 17);
        ...
        private void ProcessDefineRestartIntervalMarker(int remaining)
        {
            ...
            var bytes = this.InputProcessor.ReadFull(0, remaining);
            this.RestartInterval = (bytes[0] << 8) + bytes[1];
        }
            ...
            while (remaining > 0)
            {
                ...
                switch (x >> 4)
                {
                    case 0:
                        ...
                        bytes = this.InputProcessor.ReadFull(0, Block8x8F.ScalarCount);

                        for (int i = 0; i < Block8x8F.ScalarCount; i++)
                        {
                            this.QuantizationTables[tq][i] = bytes[i];
                        }
                    ...
                    case 1:
                        ...
                        bytes = this.InputProcessor.ReadFull(0, 2 * Block8x8F.ScalarCount);

                        for (int i = 0; i < Block8x8F.ScalarCount; i++)
                        {
                            this.QuantizationTables[tq][i] = (bytes[2 * i] << 8) | bytes[(2 * i) + 1];
                        }
        ...
        private void ProcessStartOfFrameMarker(int remaining)
        {
            ...
            this.ImageHeight = (bytes[1] << 8) + bytes[2];
            this.ImageWidth = (bytes[3] << 8) + bytes[4];
            if (bytes[5] != this.ComponentCount)
            {
            ...
            {
                this.ComponentArray[i].Identifier = bytes[6 + (3 * i)];
                ...
                this.ComponentArray[i].Selector = bytes[8 + (3 * i)];
